...
export default async function parse(buffer) {
...

    {
        ...
        while (offset < buffer.length) {
            ...
            const type = buffer.subarray((offset += 4), (offset += 4));

            const data = buffer.subarray(offset, (offset += size));

            const checksum = buffer.subarray(offset, (offset += 4));

            const block = new TextDecoder().decode(type);
    ...
    {
        const data = chunks.get("IHDR"),
            type = data[9];

        const width = dwordAt(data, 0),
