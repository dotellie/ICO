...
export default async function parse(data, png) {
    ...
    data = new Uint8Array(data.map((a) => [...a]).flat());
    ...
    if (!png.interlace) {
        const size = height * (floor((width * bpp * depth + 7) * 0.125) + 1);
    ...
    if (!data) throw `Inflate wasn't created`;

    if (!data.length) throw `Invalid inflate length`;
